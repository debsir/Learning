...
def simple_app(environ, start_response):
    gohome = '<br><a href="/">Return to home page</a>'
    ...
    if environ['PATH_INFO'] == '/':
        ...
        return pate('Welcome! Enter a string: <form action="encode">'
        ...
        if 'mystring' not in q or not q['mystring'][0]:
            start_response('400 Bad Request', [('Content-Type', 'text/plain')])
            return ['Error: this form requires a "mystring" parameters']

        my = q['mystring'][0]
        start_response('200 OK', [('Content-Type', 'text/html')])
        return page('<tt>%s</tt> base64 encoded is: <tt>%s</tt>' + gohome,
                    cgi.escape(repr(my)), cgi.escape(base64.b64decode(my)))
